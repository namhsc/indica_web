...
import { Task, TaskStatus, TaskPriority, TaskType, TaskStats, TaskHistory, TaskReminder } from '../types';
...
export function useTaskManagement({ userId, userName, userRole }: UseTaskManagementProps) {
	...
	useEffect(() => {
		...
		// Kiểm tra xem có công việc cho hôm nay và ngày mai không
		const now = new Date();
		...
		const hasTodayTasks = loadedTasks.some(
			(task) => task.dueDate === todayStr && task.assignedTo.id === userId,
		);
		const hasTomorrowTasks = loadedTasks.some(
			(task) => task.dueDate === tomorrowStr && task.assignedTo.id === userId,
		);

		// Nếu không có dữ liệu hoặc thiếu công việc cho hôm nay/ngày mai, tạo mock data
		if (loadedTasks.length === 0 || !hasTodayTasks || !hasTomorrowTasks) {
			const mockTasks = generateMockTasks(userId, userName, userRole);
			
			// Merge với dữ liệu hiện có, tránh trùng lặp
			const existingTaskIds = new Set(loadedTasks.map((t) => t.id));
			const newMockTasks = mockTasks.filter((t) => !existingTaskIds.has(t.id));
			
			// Chỉ thêm các task cho hôm nay và ngày mai nếu chưa có
			const tasksToAdd: Task[] = [];
			if (!hasTodayTasks) {
				tasksToAdd.push(...mockTasks.filter((t) => t.dueDate === todayStr));
			}
			if (!hasTomorrowTasks) {
				tasksToAdd.push(...mockTasks.filter((t) => t.dueDate === tomorrowStr));
			}
			
			// Loại bỏ trùng lặp
			const uniqueTasksToAdd = tasksToAdd.filter(
				(t) => !existingTaskIds.has(t.id),
			);
			
			loadedTasks = [...loadedTasks, ...uniqueTasksToAdd];
		}
	...
	const createTask = useCallback((
		taskData: Omit<Task, 'id' | 'createdAt' | 'updatedAt' | 'status'> & { status?: TaskStatus },
	) => {
		...
		if (newTask.reminderEnabled && newTask.reminderDate && newTask.reminderTime) {
	...
	const updateTask = useCallback((taskId: string, updates: Partial<Task>) => {
		setTasks((prev) => {
			...
			addHistory(taskId, action, Object.keys(changes).length > 0 ? changes : undefined);
	...
	const acceptTask = useCallback((taskId: string) => {
		...
		addHistory(taskId, 'accepted', undefined, 'Chấp nhận công việc được giao');
	...
	const rejectTask = useCallback((taskId: string, reason?: string) => {
		...
		addHistory(taskId, 'rejected', undefined, reason || 'Từ chối công việc được giao');
	}, [updateTask, addHistory]);
	...
	// Get user tasks
	const getUserTasks = useCallback((filter?: {
	...
	const getTaskHistory = useCallback((taskId: string) => {
		return history.filter((h) => h.taskId === taskId).sort((a, b) => 
