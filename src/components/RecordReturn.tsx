...
export function RecordReturn({ records, onReturnRecord }: RecordReturnProps) {
	...
	const completedRecords = records.filter(
		(r) => r.status === 'COMPLETED_EXAMINATION' && !r.signature,
	);

	const handleSearch = () => {
		const found = completedRecords.find(
			(r) =>
										...
										<div className="mt-2 space-y-2 text-sm">
											<div className="grid grid-cols-2 gap-x-4 gap-y-2">
												...
												<div>
													...
													<span className="ml-2">
														{selectedRecord.patient.phoneNumber}
													</span>
												</div>
												<div>
								...
								<div className="border-t pt-4">
									...
									<div className="space-y-2 text-sm">
										<div className="flex justify-between">
											...
											<span>
												{formatCurrency(selectedRecord.totalAmount || 0)}
											</span>
										</div>
										<div className="flex justify-between">
							...
							<div className="space-y-2">
								{completedRecords.slice(0, 5).map((record) => (
									...
									>
										<div>
											<div>
												{record.receiveCode} - {record.patient.fullName}
											</div>
